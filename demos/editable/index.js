...
createEditableLayers(stage, imageUrl, function (err, layers) {
  ...
  function paintOnScreenXY(screenXY) {
    ...
    var imageXY = coordinatesToEquirect(
      coordinates,
      canvas.width,
      canvas.height
    );
  ...
  function checkAndPaint(startX, startY, xDirection, yDirection, screenXY) {
    ...
    for (pixelY = startY; true; pixelY = pixelY + yDirection) {
      ...
      for (
        pixelX = startX;
        true;
        pixelX = Marzipano.util.mod(pixelX + xDirection, canvas.width)
      ) {
        if (!firstX && pixelX === startX) {
          break;
        }
        firstX = false;
  ...
  function paintIfInRange(pixelXY, screenXY) {
    var pixelCoordinates = equirectToCoordinates(
      pixelXY,
      canvas.width,
      canvas.height
    );
